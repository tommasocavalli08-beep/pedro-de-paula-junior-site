...
export function hasEditorialStorage() {
  return Boolean(
...
export async function getEditorialContent({ fresh = false, strict = false } = {}) {
  if (!hasEditorialStorage()) {
    const error = new Error('Vercel Blob não está conectado ao projeto.');
  ...
  try {
    ...
    console.error('EDITORIAL_READ_FAILED', error?.message || error);
    if (strict) throw error;
    return defaultEditorial;
  }
}

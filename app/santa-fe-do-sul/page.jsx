...
export const metadata = {
  ...
  description: 'Consultas e exames com Dr. Pedro de Paula Junior em Santa Fé do Sul (SP): cirurgia geral e digestiva, hérnias, vesícula e saúde gastrointestinal. Rua Cinco, 1198, Centro.',
    ...
    <PublicShell>
      <main id="conteudo" className="local-page">
        <section className="local-hero section-pad">
          <div data-reveal><p className="eyebrow"><span /> Santa Fé do Sul · SP</p><h1>Cirurgia digestiva em <em>Santa Fé do Sul.</em></h1><p>Consultas e exames com o Dr. Pedro de Paula Junior para avaliação cirúrgica, investigação e acompanhamento de condições gastrointestinais.</p><div className="hero-actions"><a className="btn btn-dark" href={loc.whatsapp} target="_blank" rel="noreferrer">Agendar pelo WhatsApp <Arrow /></a><a className="btn btn-ghost" href={loc.maps} target="_blank" rel="noreferrer">Abrir no mapa</a></div></div>
          <div className="address-panel" data-reveal><span>Consultas e exames</span><h2>{loc.streetAddress}</h2><p>Centro · {loc.city} - {loc.state}<br/>CEP {loc.postalCode}</p><dl><div><dt>Telefone</dt><dd>{loc.phoneDisplay}</dd></div><div><dt>Horário</dt><dd>{editorial.practical.santaFeHours}</dd></div><div><dt>E-mail</dt><dd>{doctor.email}</dd></div></dl><small>{loc.note}</small></div>
        </section>
